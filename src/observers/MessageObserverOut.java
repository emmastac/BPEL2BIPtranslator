...
public class MessageObserverOut extends BIPPropertyObserver {
	...
	public MessageObserverOut( String name , TemplateMaker tm ) {
	...
	public void post_observeActivity( Element el , ArrayDeque < HashMap < String , ArrayList >> stack , HashMap < String , ArrayList > ret )
		...
		String name = null;
		if ( NodeName.getNodeName( el.getTagName( ) ).equals( NodeName.INVOKE ) && !( name = el.getAttribute( "name" ) ).equals( "" ) ) {
			if ( !req_to_resp.containsKey( name ) ) {
				return;
			}
		...
		} else if ( ( NodeName.getNodeName( el.getTagName( ) ).equals( NodeName.RECEIVE ) || NodeName.getNodeName( el.getTagName( ) ).equals( NodeName.ON_MESSAGE ) )&& !( name = el.getAttribute( "name" ) ).equals( "" ) ) {
			if ( !resp_to_req.containsKey( name ) ) {
				return;
			}
	...
	private void applyObserver( Element el , ArrayDeque < HashMap < String , ArrayList >> stack , HashMap < String , ArrayList > ret )
		...
		StringTemplate template = AttrFiller.getTemplate( "/resources/msgObserver.stg" , "OBSERVER" );
		template.setAttribute( "compName" , MapUtils.getMapSingleEntry( ret , MapKeys.CHILD_COMP ) );

		String childComp = "bpelProcess_msgout_obs";
	...
	public boolean configure( String bpelFile , String sourceHome ) throws Exception {
		...
		while ( ( line = in.readLine( ) ) != null && !line.equals("") ) {
			...
			if ( requestDirection.equals( "out" ) ) {
				found_one = true;
